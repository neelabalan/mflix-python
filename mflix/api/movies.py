...
movies_api_v1 = Blueprint("movies_api_v1", "movies_api_v1", url_prefix="/api/v1/movies")
...
def api_search_movies():
    ...
    (movies, total_num_entries) = get_movies(filters, page, DEFAULT_MOVIES_PER_PAGE)
...
def api_search_movies_faceted():
    ...
    # finally use the database and get what is necessary
    try:
        (movies, total_num_entries) = get_movies_faceted(filters, page, MOVIES_PER_PAGE)
...
def api_update_comment():
    ...
    try:
        ...
        updated_comment = expect(
            post_data.get("updated_comment"), str, "updated_comment"
        )
        movie_id = expect(post_data.get("movie_id"), str, "movie_id")
