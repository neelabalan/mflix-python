...
def register():
    ...
    insertionresult = add_user(
        name,
        email,
        bcrypt.generate_password_hash(password=password.encode("utf8")).decode("utf-8"),
    )
...
def login():
    ...
    userdata = get_user(email)
    if not userdata:
        response_object = {"error": {"email": "Make sure your email is correct."}}
        return make_response(jsonify(response_object)), 401
    if not bcrypt.check_password_hash(userdata["password"], password):
        response_object = {"error": {"password": "Make sure your password is correct."}}
        return make_response(jsonify(response_object)), 401
...
def delete():
    ...
    try:
        ...
        if not user.email == userdata["email"] and not bcrypt.check_password_hash(
