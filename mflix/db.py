...
def get_movies(filters, page, movies_per_page):
    ...
    skip_distance = page * movies_per_page
    # skip_distance = movies_per_page * page
    if page == 0:
        movies = cursor.limit(movies_per_page)
    else:
        movies = cursor.skip(skip_distance).limit(movies_per_page)
            ...
            {"$match": {"_id": ObjectId(id)}},
            {
                "$lookup": {
                    ...
                    "pipeline": [
                        {
                            "$match": {
                                "$expr": {"$eq": ["$movie_id", ObjectId(id)]}  #'$_id'
                            },
                        },
                        {"$sort": {"date": -1}},
                    ],
...
def login_user(email, jwt):
    ...
    try:
        ...
        db.sessions.update_one(
            {"user_id": email}, {"$set": {"user_id": email, "jwt": jwt}}, upsert=True
        )
        return {"success": True}
    except Exception as e:
        return {"error": e}
